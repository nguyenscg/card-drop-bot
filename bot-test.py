...
async def on_ready():
    ...
    # Only load global collection if necessary, avoid overwriting user data
    global collection
    try:
        with open("collection.json", "r") as f:
            collection = json.load(f)  # This is your global collection
            print("Global collection loaded:", collection)
    except FileNotFoundError:
        print("collection.json not found. Creating a new one.")
        # Create a new collection with predefined data if the file does not exist
        collection = [{"name": "Example", "group": "Group", "image": "url"}]  # Example data

        
        # Save the empty collection to the file
        with open("collection.json", "w") as f:
            json.dump(collection, f, indent=4)
        
        print("New collection.json created.")
    except json.JSONDecodeError:
        print("Error: Failed to decode 'collection.json'. Make sure it contains valid JSON.")
